...
void OS_Init(void){
  ...
  //1 ms hardware interrupt to decrement sleep;
...
void SetInitialStack(int i){
  ...
  Stacks[i][STACKSIZE-1] = 0x01000000; // Thumb bit (PSR)
	// Stacks[i][STACKSIZE-2] is the PC that is initialized in...
...
void static runperiodicevents(void){
	...
	// In Lab 4, handle periodic events in RealTimeEvents
  	uint16_t i;
	// decrease the sleep counters for threads in sleep mode
...
void OS_Signal(int32_t *semaPt){
  ...
  if((*semaPt) <= 0)
	{
		// search for a thread blocked on this semaphore
    pt = RunPt->next;
...
void OS_EdgeTrigger_Init(int32_t *semaPt, uint8_t priority){
	...
	SYSCTL_RCGCGPIO_R |= (0x1 << SYSCTL_RCGCGPIO_R3 );
	// allow time for clock to stabilize
	while((SYSCTL_PRGPIO_R&0x08) == 0){}; // allow time for clock to stabilize
	OS_InitSemaphore(edgeSemaphore,0);  // initialize semaphores
	// 2) no need to unlock PD6
	// 3) disable analog on PD6
	GPIO_PORTD_AMSEL_R &= ~(0x1 << 6);
	// 4) configure PD6 as GPIO
		GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0xF0FFFFFF);
	// 5) make PD6 input
	GPIO_PORTD_DIR_R &= ~(0x1 << 6);
	// 6) disable alt funct on PD6
	GPIO_PORTD_AFSEL_R &= ~(0x1 << 6);
	// disable pull-up on PD6
	GPIO_PORTD_PUR_R &= ~(0x1 << 6);
	// 7) enable digital I/O on PD6
	GPIO_PORTD_DEN_R |= (0x1 << 6); 
	// (d) PD6 is edge-sensitive	
	//     PD6 is not both edges 
	//     PD6 is falling edge event
	GPIO_PORTD_IS_R &= ~(0x1 << 6);
	GPIO_PORTD_IBE_R &= ~(0x1 << 6);
	GPIO_PORTD_IEV_R &= ~(0x1 << 6);
	// (e) clear PD6 flag
	GPIO_PORTD_ICR_R |= (0x1 << 6);
	// (f) arm interrupt on PD6
	GPIO_PORTD_IM_R |= (0x1 << 6);
